...
import {
  onChangeEndTime,
  onChangeStartTime,
} from "../../Component/Search/onChangeTimes";
...
const Table = ({
  ...
  const accData = useTableGraphStore((state) => state.accData);
  ...
  const isAccValue = (
    sensorData: string | AccValue
  ): sensorData is AccValue => {
    // sensorData가 객체이고, average 속성을 가지고 있는지 확인
    return (
      typeof sensorData === "object" &&
      sensorData !== null &&
      "average" in sensorData
    );
  };
  ...
  const data = useMemo(() => {
    ...
    return reversedUpdatedModels.map((tableAndGraph) => {
      ...
      for (let i = 0; i <= 20; i++) {
        const sensorKey = `s${i}`;
        const sensorData = tableAndGraph[sensorKey as keyof TableAndGraph] as
          | AccValue
          | string;
        if (isAccValue(sensorData)) {
          result.push(sensorData.average.toFixed(1));
    ...
    <div className="table">
      <div className="table__header">
        ...
        <div className="table__search">
            ...
            <div className="table-search__search--wrap">
              ...
              <label htmlFor="startTimeForTable">
                <div className="table-search__search--iconBox">
                  <img
                    src="/assets/icon/monitoring/ic_clock.png"
                    className="timeIconBoxImg"
                    alt="시계 이미지"
                  />
                </div>
              </label>
          ...
          <button className="table__search-button" onClick={onClickSearch}>
            조회
          </button>
        </div>
      </div>
          ...
          <table className="table__table">
            <thead className="table__table-head">
              <tr className="table__head-line">
                <td className="table__head-time" key="time">
                  측정시간
                </td>
                {device?.sensors?.map((sensor) => {
                  return (
                    <td className="table__head-item" key={sensor.spec?.id}>
                      {sensor.spec?.name}
                    </td>
                  );
                })}
