...
import getAverageValues from "../../Component/AverageValues";
...
const Graph = ({
  ...
  const selectedSensor = useSelectedSensorStore(
    (state) => state.selectedSensor
  );

  const [label, calcData] = useMemo(() => {
    if (
      accData === null ||
      selectedSensor?.index === null ||
      selectedSensor === null
    ) {
      return [[], []];
    }
    ...
    accData.tableAndGraph.forEach((data) => {
      const averages = getAverageValues(data);
      if (
        averages &&
        selectedSensor.index !== undefined &&
        averages[selectedSensor.index] !== undefined
      ) {
    ...
    <div className="graph">
      <div className="graph__header">
        ...
        <div className="graph__search">
          ...
          <button className="graph__search-button" onClick={onClickSearch}>
            조회
          </button>
        </div>
      </div>
